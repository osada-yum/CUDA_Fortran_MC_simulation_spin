program xy2d_gpu_over_relaxation
  ...
  integer(int32), parameter :: tot_sample = 500_int32
  integer(int64), parameter :: nx = 10001_int64
  integer(int64), parameter :: ny = nx - 1
  ...
  integer(int32), parameter :: mcs_over_relax = 10000
  integer(int32), parameter :: n_over_relax = 1
  ...
  do i = 1, size(outs)
     ...
     write(outs(i), '(a, i0)' ) "# mcs_over_relax: ", mcs_over_relax
     write(outs(i), '(a, i0)' ) "# n_over_relax: ", n_over_relax
  ...
  do sample = 1, tot_sample
     ...
     do i = 1, mcs
        call xy2d%update()
        if (i <= mcs_over_relax) &
             & call xy2d%update_over_relaxation(n_over_relax)
...
end program xy2d_gpu_over_relaxation
