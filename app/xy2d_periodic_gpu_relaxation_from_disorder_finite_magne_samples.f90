program xy2d_periodic_gpu_relaxation_from_disorder_finite_magne_samples
  ...
  real(real64), parameter :: finite_magne = 0.02d0
  ...
  do i = 1, size(outs)
     ...
     write(outs(i), '(*(g0, 1x))' ) "# Initail finite magne", finite_magne
  end do

  write(output_unit, '(*(g0, 1x))') "# N, smaple, time, m_x, e, m_y, A"
  do sample = 1, tot_sample
     call xy2d%set_finite_magne_spin(finite_magne)
...
end program xy2d_periodic_gpu_relaxation_from_disorder_finite_magne_samples
