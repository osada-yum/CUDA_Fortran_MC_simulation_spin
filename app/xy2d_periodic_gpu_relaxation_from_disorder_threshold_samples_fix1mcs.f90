program xy2d_periodic_gpu_relaxation_from_disorder_threshold_fix
  ...
  real(real64), parameter :: threshold = n_inv_r64
  ...
  do i = 1, size(outs)
     ...
     write(outs(i), '(*(g0, 1x))' ) "# Threshold", threshold
     write(outs(i), '(*(g0, 1x))' ) "# test Fix 1MCS magne as x-aligned"
  end do

  write(output_unit, '(*(g0, 1x))') "# N, smaple, time, m_x, e, m_y, A"
  do sample = 1, tot_sample
     call xy2d%set_random_small_spin(threshold)
     call xy2d%set_initial_magne_autocorrelation_state()

     write(error_unit, '(*(g0, 1x))') "#", sample, xy2d%calc_magne_sum() * n_inv_r64, xy2d%calc_autocorrelation_sum() * n_inv_r64
...
end program xy2d_periodic_gpu_relaxation_from_disorder_threshold_fix
