...
output_dir="data/xy2d_periodic_over_relaxation"
outputfile="${output_dir}/xy2d_periodic_GPU_x${nx}_y${ny}_mcs${mcs}_sample${sample}_kbt${kbt}_iseed${iseed}_skip${n_skip}_mcs_over_relax${mcs_over_relax}_n_over_relax${n_over_relax}_${execname}_$(date +%Y%m%d_%H%M%S).dat"
mkdir -p "${output_dir}"
tmpdir="data/tmp"
mkdir -p "${tmpdir}"
...
echo "xy2d_MET_GPU(${execname}),  ${nx}x${ny} == $((nx*ny)), ${sample}, ${mcs}, ${kbt}, iseed ${iseed}, mcs_over_relax${mcs_over_relax}, n_over_relax${n_over_relax}, time ${elapsed_time}, ${outputfile}" | tee -a gpu_xy2d_periodic_over_realxatino.log
