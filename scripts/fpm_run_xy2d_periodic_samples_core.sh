...
sed -i \
    -e "s/NUM_THREADS = [0-9]*/NUM_THREADS = ${NUM_THREADS}/" \
    "${srcfile}"
...
echo "xy2d_MET_GPU(${execname}),  ${nx}x${ny} == $((nx*ny)), ${sample}x${NUM_THREADS} == $((sample*NUM_THREADS)), ${mcs}, ${kbt}, iseed ${iseed}, time ${elapsed_time}, ${outputfile}" | tee -a gpu_xy2d_periodic.log
