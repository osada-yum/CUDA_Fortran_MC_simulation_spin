...
root_dir="/tmp"
progfile="./app/xy2d_gpu_relaxations.f90"
execfile="${root_dir}/bin/xy2d_gpu_relaxations"
outputdir="data/2D-XY_relaxations"
outputfile="${outputdir}/2D-XY_GPU_x${nx}_y${ny}_mcs${mcs}_bin${bin}_sample${sample}_kbt${kbt}_iseed${iseed}_skip${n_skip}_$(date +%Y%m%d_%H%M%S).dat"
mkdir -p ${outputdir}
tmpdir="data/tmp"
mkdir -p "${tmpdir}"
...
echo "2D-XY_MET_GPU_relaxations,  ${nx}x${ny} == $((nx*ny)), bin${bin}_sample${sample}, ${mcs}, ${kbt}, iseed ${iseed}, time ${elapsed_time}, ${outputfile}" | tee -a gpu_xy2d_relaxations.log
