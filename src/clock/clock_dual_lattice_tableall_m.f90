module clock_dual_lattice_tableall_gpu_m
  use, intrinsic :: iso_fortran_env
  use cudafor
  use curand
  implicit none
  private
  character(len=*), parameter :: version = "GPU_dual_lattice_tableall"
  ...
  type(dim3), parameter :: NUM_BLOCK = dim3(&
       & (nx / 2 + NUM_THREADS%x - 1) / NUM_THREADS%x, &
...
contains
 ...
 impure subroutine skip_curand_clock(n_skip)
  ...
  attributes(global) pure subroutine update_sub(sixclock_update, sixclock_nearest, rnds, parity_bit)
    integer(int32), intent(inout) :: sixclock_update(nx/2, ny)
    integer(int32), intent(in) :: sixclock_nearest(nx/2, ny)
    real(real64), intent(in) :: rnds(2, nx, ny)
    integer(int32), value :: parity_bit
    integer(int32) :: new_state
    integer(int64) :: x, y
    integer(int64) :: rx, lx, uy, dy, actual_x
    ...
    !> right
    rx = x + iand(y + 1 + parity_bit, b'1')
    if (rx > nx / 2) rx = 1
    !> left
    lx = x - iand(y + parity_bit, b'1')
    if (lx < 1) lx = nx / 2
    !> up
    uy = y + 1
    if (uy > ny) uy = 1
    ...
    actual_x = 2 * x - iand(y + parity_bit, b'1')
    new_state = sixclock_update(x, y) + ceiling(rnds(1, actual_x, y) * (mstate - 1))
  ...
  contains
    pure real(real64) function calc_magne_sub(sixclock_even, sixclock_odd) result(res)
      integer(int32), intent(in), device :: sixclock_even(nx/2, ny), sixclock_odd(nx/2, ny)
      integer(int64) :: y, x
      res = 0d0
  ...
  contains
    pure real(real64) function calc_energy_sub(sixclock_even, sixclock_odd) result(res)
      integer(int32), intent(in), device :: sixclock_even(nx/2, ny), sixclock_odd(nx/2, ny)
      integer(int64) :: y, x, rx, uy, addition_even, addition_odd
      res = 0d0
      !$acc parallel loop private(y, x, rx, uy, addition_even, addition_odd) reduction(+:res)
      do y = 1, ny
         uy = y + 1
         if (uy > ny) uy = 1
         addition_even = iand(y + 1, b'1')
         addition_odd = iand(y, b'1')
         do x = 1, nx / 2
            !> even
            rx = x + addition_even
            if (rx > nx / 2) rx = 1
...
end module clock_dual_lattice_tableall_gpu_m
