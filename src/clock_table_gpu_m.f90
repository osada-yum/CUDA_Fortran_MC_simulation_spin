module clock_table_gpu_m
  use, intrinsic :: iso_fortran_env
  use cudafor
  use curand
  implicit none
  private
  character(len=*), parameter :: version = "GPU_table"
...
contains
 ...
 impure subroutine skip_curand_clock(n_skip)
  ...
  attributes(global) pure subroutine update_sub(sixclock, rnds, parity_lattice)
    ...
    delta_e = state_center_right_up_to_energy(new_state, nearest_spins(1), nearest_spins(3)) &
         & - state_center_right_up_to_energy(sixclock(x, y), nearest_spins(1), nearest_spins(3)) &
         & + state_center_right_up_to_energy(new_state, nearest_spins(2), nearest_spins(4)) &
         & - state_center_right_up_to_energy(sixclock(x, y), nearest_spins(2), nearest_spins(4))
    if (delta_e > 0) then
