...
module xy2d_periodic_yhalf_flip_spin_index_gpu_m
...
contains
  impure subroutine init_xy2d_gpu(this, nx, ny, kbt, iseed)
    ...
    allocate(this%spins_even_(1:2, 0 : this%nx_ + 1, 0:this%ny_ / 2 + 1))
    allocate(this%spins_odd_(1:2, 0 : this%nx_ + 1, 0:this%ny_ / 2 + 1))
 ...
 impure subroutine skip_curand_xy2d_gpu(this, n_skip)
  ...
  attributes(global) pure subroutine set_allup_spin_sub(nall, nx, ny, spins)
    ...
    real(real64), intent(inout) :: spins(1:2, 0 : nx + 1, 0 : ny / 2 + 1)
  ...
  attributes(global) pure subroutine update_norishiro_leftright_sub(nall, nx, ny, spins)
    ...
    spins(1:2, 0, y) = spins(1:2, nx, y)
    ! norishiro right, (nx + 1, 1:ny/2) <- (1, 1:ny/2)
    spins(1:2, nx + 1, y) = spins(1:2, 1, y)
  ...
  attributes(global) pure subroutine update_sub(nall, nx, ny, spins_update, spins_around, beta, randoms, candidates, even_or_odd)
    integer(int64), value :: nall, nx, ny
    real(real64), intent(inout) :: spins_update(1:2, 0 : nx + 1, 0 : ny / 2 + 1)
    real(real64), intent(in) :: spins_around(1:2, 0 : nx + 1, 0 : ny / 2 + 1)
  ...
  attributes(global) pure subroutine over_relaxation_sub(nall, nx, ny, spins_update, spins_around, even_or_odd)
    ...
    spins_update(1:2, x, y) = (2 * sum(local_field(1:2) * spins_update(1:2, x, y))) * local_field(1:2) - spins_update(1:2, x, y)
    block
      real(real64) :: rabs
      rabs = hypot(spins_update(x, y, 1), spins_update(x, y, 2))
      spins_update(1:2, x, y) = spins_update(1:2, x, y) / rabs
    end block
  ...
  attributes(device) pure function calc_local_field(nx, ny, spins_around, x, y, even_or_odd) result(res)
    ...
    do d = 1, nd / 2
       ...
       res(1:2) = res(1:2) + spins_around(1:2, x, near_y)
    end do
    do d = nd / 2 + 1, nd
       near_x = x + dx(d)
       res(1:2) = res(1:2) + spins_around(1:2, near_x, y)
    end do
  ...
  attributes(device) pure real(real64) function calc_delta_energy(nx, ny, spins_update, spins_around, x, y, candidate, even_or_odd) result(res)
    integer(int64), value :: nx, ny, x, y
    real(real64), intent(in) :: spins_update(1:2, 0 : nx + 1, 0 : ny / 2 + 1)
    real(real64), intent(in) :: spins_around(1:2, 0 : nx + 1, 0 : ny / 2 + 1)
  ...
  pure real(real64) function calc_energy_sum_xy2d_gpu_sub(nall, nx, ny, spins) result(res)
    integer(int64), value :: nall, nx, ny
    real(real64), intent(in), device :: spins(1:2, 0 : nx + 1, 0 : ny / 2 + 1)
    integer(int64) :: idx, x, y
    res = 0d0
    ...
    do idx = 1, nall / 2
       y = (idx - 1) / nx + 1
       x = idx - (y - 1) * nx
       res = res - spins(1, x, y) * (spins(1, x + 1, y) + spins(1, x, y + 1))
       res = res - spins(2, x, y) * (spins(2, x + 1, y) + spins(2, x, y + 1))
    end do
...
end module xy2d_periodic_yhalf_flip_spin_index_gpu_m
